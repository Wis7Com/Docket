...
import { generateDocumentOutline, getApiBase } from "@/app/lib/docketApi";
import {
    buildHeuristicDocxOutline,
    restoreDocxOutline,
...
function docxHeadingLevel(el: HTMLElement): number | null {
    const tagLevel = /^h([1-6])$/i.exec(el.tagName)?.[1];
    if (tagLevel) return Number(tagLevel);
    for (const className of Array.from(el.classList)) {
        const match = /(?:^|_|-)heading(?:_|-)?([1-6])$/i.exec(className);
        if (match?.[1]) return Number(match[1]);
    }
    return null;
}

function collectDocxNavigation(root: HTMLElement): DocumentNavigationItem[] {
    ...
    candidates.forEach((el) => {
        ...
        const id = `docx-${base}${count ? `-${count + 1}` : ""}`;
        el.setAttribute(NAV_ATTR, id);
...
export function DocxView({
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (cancelled) return;
                const builtInNavItems = collectDocxNavigation(containerEl);
                const nextNavItems = builtInNavItems.length
                    ? builtInNavItems
                    : restoreDocxOutline(
                          containerEl,
                          NAV_ATTR,
                          loadGeneratedDocumentOutline(
                              window.localStorage,
                              generatedOutlineKeyRef.current,
                          ),
                      );
                setNavItems(nextNavItems);
                setActiveNavId(nextNavItems[0]?.id ?? null);
                setNavGenerateMessage(null);
    ...
    useEffect(() => {
        const scrollEl = scrollRef.current;
        const containerEl = containerRef.current;
        if (!scrollEl || !containerEl || navItems.length === 0) return;
        let scheduled = false;
        const updateActive = () => {
            scheduled = false;
            const scrollRect = scrollEl.getBoundingClientRect();
    ...
    async function generateHeuristicNavItems() {
        const containerEl = containerRef.current;
        if (!containerEl || navGenerationBusyRef.current) return;
        ...
        setNavGenerateMessage(null);
        try {
            let items = buildHeuristicDocxOutline(containerEl, NAV_ATTR);
            let fallbackMessage: string | undefined;
            if (!items.length) {
                const fallback = await generateDocumentOutline(documentId, {
                    versionId,
                    model: outlineModel,
                });
                if (!isCurrent()) return;
                items = restoreDocxOutline(
                    containerEl,
                    NAV_ATTR,
                    fallback.items,
                );
    ...
    function scrollToNavItem(item: DocumentNavigationItem) {
        ...
        scrollEl.scrollTo({
            top: Math.max(
                0,
                scrollEl.scrollTop + targetRect.top - scrollRect.top - 24,
            ),
            behavior: "smooth",
        });
