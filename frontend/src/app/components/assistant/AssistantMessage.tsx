...
function ResponseStatus({ status }: { status: StatusState }) {
    ...
    useEffect(() => {
        ...
        if (wasActive && !isActive) {
            ...
            const hideTimer = setTimeout(() => setDoneVisible(false), 1500);
            return () => {
    ...
    return (
        <div className="w-full h-9 flex items-center mb-2">
            <DocketIcon
                ...
                error={isError}
                docket={!isError && !(showDone && doneVisible)}
                size={22}
            />
        </div>
    );
...
export function AssistantMessage({
    ...
    const copyableResponseText =
        (processedTexts.length > 0
            ? processedTexts.filter((text) => text.trim()).join("\n\n")
            : content
        ).trim() ||
    ...
    const handleCopy = async () => {
        ...
        try {
            await navigator.clipboard.writeText(copyableResponseText);
            setIsCopied(true);
        ...
        <div style={{ minHeight }}>
            ...
            <div className="w-full font-inter relative mt-2">
                ...
                <div className="flex items-center gap-2 pt-2 pb-4 md:pb-8 font-sans justify-start">
                    {!isStreaming && copyableResponseText && (
                        <button
                            type="button"
                            className="inline-flex h-8 items-center gap-1.5 rounded-md border border-gray-200 bg-white px-2.5 text-xs font-medium text-gray-600 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-900"
                            onClick={handleCopy}
