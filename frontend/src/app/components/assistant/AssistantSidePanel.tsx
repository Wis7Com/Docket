...
export function AssistantSidePanel({
    ...
    const activeIndex = active ? tabs.findIndex((t) => t.id === active.id) : -1;
    const activeId = active?.id ?? null;

    const tabStripRef = useRef<HTMLDivElement>(null);

    // Keep the active tab visible when it changes (chevron navigation can
    // land on a tab that is scrolled out of the strip).
    useEffect(() => {
        if (!activeId) return;
        const el = tabStripRef.current?.querySelector(
            `[data-side-tab-id="${CSS.escape(activeId)}"]`,
        );
    ...
    const activateByOffset = (delta: number) => {
        const next = tabs[activeIndex + delta];
        if (next) onActivateTab(next.id);
    };
    ...
    return (
        ...
        >
            ...
            <div className="flex items-end gap-1 pr-2 pt-2 bg-gray-100">
                {tabs.length > 1 && (
                    <div className="ml-1.5 flex shrink-0 items-center self-center overflow-hidden rounded-md border border-gray-300 bg-white shadow-sm">
                        <button
                            type="button"
                            onClick={() => activateByOffset(-1)}
                            disabled={activeIndex <= 0}
                            className="flex h-7 w-7 items-center justify-center border-r border-gray-300 text-gray-700 transition-colors hover:bg-gray-200 hover:text-gray-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-300"
                            title="Previous document tab"
                            aria-label="Previous document tab"
                            data-session-check="side-panel-prev-tab"
                        >
                        ...
                        <button
                            ...
                            className="flex h-7 w-7 items-center justify-center text-gray-700 transition-colors hover:bg-gray-200 hover:text-gray-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-300"
                            title="Next document tab"
                            aria-label="Next document tab"
                            data-session-check="side-panel-next-tab"
                        >
                ...
                <div
                    ref={tabStripRef}
                    className="flex-1 flex items-end gap-1 overflow-x-auto pl-2 pr-2"
                >
                    {tabs.map((tab) => {
