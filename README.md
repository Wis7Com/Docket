...
the first two work on an ordinary CPU, the next two need a language
model — a lightweight local one is enough. See
...
Turn embeddings on and the same index also answers by meaning. The
default embedding model is `batiai/qwen3-embedding:0.6b` via Ollama —
small enough to run on a CPU — and vectors are stored in the project's
own database. Lexical and semantic hits are merged with reciprocal rank
fusion, and every result carries a badge saying *why* it matched.
...
This ran on a local **Ollama `gemma4:26b-a4b-it-q4_K_M`** — a
lightweight local model, no GPU required, no API key, no network:
...
**The one GPU exception.** A GPU is never required for any of the
above — local models run on CPU, a GPU just makes them faster. The
single deliberate exception is *LLM-generated* document outlines, which
Docket only attempts when hardware acceleration is detected, to avoid
multi-minute stalls; without a GPU it keeps the heuristic outline
(visible in the viewer screenshots above).
