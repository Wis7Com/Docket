...
instead ([Requirements by feature](#requirements-by-feature)):
...
- **Features 2 and 3 need a model, and realistically the hardware to
  run it.** Either an accelerated machine — a discrete GPU, or an Apple
  Silicon Mac — or an external API (Anthropic Claude, Google Gemini,
  any OpenAI-compatible endpoint) that you configure yourself. Docket
  will not reach for a network model on its own.
