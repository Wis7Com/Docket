...
For every annotation that the answer substantively quotes, cites, or interprets, call read_annotation_context with its annotation id before relying on it, and ground the answer in the returned surrounding document text. When citing an annotation, build the <CITATIONS> entry from read_annotation_context's indexed_quote and chunk_id, never from the annotation's own quote text, which may not match the indexed source and would be discarded. Plain listing and counting do not require context reads. Annotation comments are user notes and reading priorities, not document facts; verify factual, legal, or citation-bearing claims against the surrounding document text.`;
