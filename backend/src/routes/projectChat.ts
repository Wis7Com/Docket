...
import { buildColorLegendPrompt } from "../lib/colorLegend";
...
WHOLE-DOCUMENT SUMMARY RULE:
Whenever the user asks to summarize, review, outline, or explain an entire selected document, call summarize_document for that document. This is mandatory for long documents. It processes every indexed chunk in page order and reports explicit coverage. Never treat a single generic search_project_documents or find_in_document result as a whole-document summary, and never claim complete coverage when summarize_document reports an OCR or indexing gap. Use the prepared summary and its source-exact citations rather than inventing replacement evidence.
...
When the user asks to cover every highlight of a specific color, the get_annotation_digest summary.total and the number of numbered answer sections must match. Retrieve every required digest cursor window before answering. Open the answer by stating how many of the total annotations are covered, using the actual counts. Then provide exactly one numbered section for each annotation, containing the highlight's original text followed by the opposing party's response; when a search finds no response, say so explicitly for that item. Do not merge or omit annotations. State once that the highlights represent the user's reading priorities while the factual findings rest on source-verified citations. Write all of this in the language of the user's most recent message — the sentences above describe what to say, not the words to copy.
...
function projectToolsForCorpus(smallIndexedCorpus: boolean): unknown[] {
    ...
    return PROJECT_EXTRA_TOOLS.filter((tool) => {
        ...
        return (
            name !== "search_project_documents" && name !== "read_index_chunk"
        );
    });
}
...
export function describeAnnotationColor(
    color: string | null | undefined,
): string | null {
    return classifyAnnotationColor(color)?.label ?? null;
}

export function requestedAnnotationColorFamilies(
    text: string,
...
export function resolveSelectedDocumentScope(
    ...
    if (!Array.isArray(rawSelectedDocumentIds)) {
        return {
            warning: "selected_document_ids was not an array; ignoring scope",
        };
    }
...
export async function buildRequestedAnnotationContext(args: {
    ...
    for (const [label, info] of Object.entries(args.docIndex)) {
        ...
        currentVersionByDocumentId.set(
            info.document_id,
    ...
    const requestedColors = requestedAnnotationColorFamilies(
        args.latestUserText ?? "",
    );
    const sortedRows = [...uniqueRows.values()]
        .filter((row) => {
            ...
            const currentVersion = currentVersionByDocumentId.get(
                row.document_id,
            );
            return (
                !row.version_id ||
                !currentVersion ||
                row.version_id === currentVersion
            );
        })
        .sort((a, b) => {
            ...
            const aHasText =
                clipAnnotationText(a.quote) || clipAnnotationText(a.comment);
            const bHasText =
                clipAnnotationText(b.quote) || clipAnnotationText(b.comment);
    ...
    const colorFilteredRows =
        requestedColors.size > 0
            ...
            : sortedRows;
    const rows = (
        colorFilteredRows.length > 0 ? colorFilteredRows : sortedRows
    ).slice(0, 30);
    ...
    const lines = rows.map((row) => {
        ...
        const filename =
            filenameByDocumentId.get(row.document_id) ?? "Unknown document";
...
export async function buildProjectColorLegendPrompt(args: {
    db: ReturnType<typeof createServerSupabase>;
    projectId: string;
}): Promise<string | null> {
    ...
    const rows = (data ?? []) as ProjectColorLegendRow[];
    return buildColorLegendPrompt(
        rows.map((row) => ({
    ...
    try {
        ...
        const smallIndexedCorpus =
            ...
            indexStats.total_documents <=
                retrievalSettings.chat_full_read_max_docs &&
        ...
        const colorLegendPrompt = await buildProjectColorLegendPrompt({
            db,
            projectId,
        });
        if (colorLegendPrompt) {
            systemPromptExtra += `\n\n${colorLegendPrompt}`;
        }
