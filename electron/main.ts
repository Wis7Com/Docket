...
async function inspectProjectPdfExportButtonDom(
  ...
  // The check clicks Export PDF for real, so keep the native save dialog out
  // of it — the renderer sees a plain cancel and the check stays headless.
  suppressSavePdfDialog = true;
  const inspection = w.webContents.executeJavaScript(
    `
      (async () => {
        const docId = ${JSON.stringify(docId)};
        const token = await window.docket?.getToken?.();
...
// Where the last export landed. Exports usually go to the same place, so
// the save dialog reopens there instead of resetting to Documents.
let lastExportDirectory: string | null = null;

// The startup smoke check clicks Export PDF for real. Suppress the save
// dialog for that window so the check never blocks on a modal it can't
// dismiss; the renderer treats the suppressed save as a plain cancel.
let suppressSavePdfDialog = false;

function toSaveBuffer(data: unknown): Buffer | null {
...
function sanitizeSaveFilename(name: unknown): string {
  const base = path
...
// Export PDF: the renderer hands over the generated bytes and the user picks
// the destination. A `<a download>` would silently drop the file into the OS
// download folder, which is the wrong default for an explicit export.
ipcMain.handle(
  "docket:savePdfAs",
  async (event, payload?: { suggestedName?: string; data?: unknown }) => {
    if (suppressSavePdfDialog) return { ok: false, canceled: true };
    const bytes = toSaveBuffer(payload?.data);
    if (!bytes || bytes.length === 0) {
      return { ok: false, error: "No PDF data to save." };
    }
    const defaultPath = path.join(
      lastExportDirectory ?? app.getPath("documents"),
      sanitizeSaveFilename(payload?.suggestedName),
    );
    ...
    try {
      ...
      return {
        ok: false,
        error: err instanceof Error ? err.message : "Failed to write the PDF.",
      };
    }
    lastExportDirectory = path.dirname(result.filePath);
